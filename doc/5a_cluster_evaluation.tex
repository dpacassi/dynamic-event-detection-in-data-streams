...
We look the five different preprocessing methods as described in \subsecref{subsec:3_text_preprocessing} and evaluate each.
The methods are:
...
\tabref{tab:cluster_parameters} shows the resulting MP-Score for each parameter in combination with each preprocessing method and vector space model.
...
\figref{fig:hdbscan_parameters} shows the scores achieved with different parameters over an increasingly large set of samples.
...
However it is important to note, that the tf-idf model in \tabref{tab:clustering_example_features} was created from only those 20 articles
...
and \textit{k}-means in a small loss regarding the precision as can be seen in \figref{fig:precision_and_processing_time_kmeans_hdbscan}
