...
Looking at an increased batch size of 5000 in Figure \ref{fig:event_detection_mp_score_5000}, we note that there is less overall variance than with a batch size of 1000, but the variance is still fairly large for detected events. Additionally while the variance is large, the median for new topics is mostly below 0.2. This tells us that most of the newly detected events do not correspond with new events according to the ground truth. The detection of extensions of existing events is generally more accurate with a median between 0.5 and 0.8, but there is still are wide variance noticeable.  
...
The difference in the accuracy of the detection of new events and the extension of events can be explained by bla bla bla,
