...
The test scenarios in \tabref{tab:score_scenarios} show the resulting scores of our similarity score, NMI and ARI.
...
As a final note, repeating the evaluation shown in \tabref{tab:nmi_kmeans_example} a second time using the MP-Score,
the score (\tabref{tab:avg_predict_kmeans_example}) for \textit{k}-means is much lower than HDBSCAN.
...
The parameters for each available clustering method are defined beforehand in a dictionary as can be seen in \lstref{lst:cluster_method_parameters}.
