...
id_column = "id"
# content_column = "newspaper_keywords"
...
#    distances = cosine_distances(raw_hash_matrix)
    ...
    #     tfidf_matrix=None
    # )
    y_true = LabelEncoder().fit_transform(test_data[story_column])
    ...
    #     print('Topic: %s' % topic)
    #     for item in news:
    #         print("Probability: {} Headline: {}".format(item[0], item[1].headline))
    #     print("------------------------------")

    y_true = LabelEncoder().fit_transform(test_data[story_column])

    # y_predict = LabelEncoder().fit_transform(y_predict)

    # print(y_true)
    # print(y_predict)
    # print(labels)

    print("HDBSCAN Completeness: %0.3f" % completeness_score(y_true, labels))
    print("HDBSCAN + LDA Completeness: %0.3f" % completeness_score(y_true, y_predict))

    print("HDBSCAN NMI score: %0.3f" % normalized_mutual_info_score(y_true, y_predict, average_method='arithmetic'))
    print("HDBSCAN + LDA NMI score: %0.3f" % normalized_mutual_info_score(y_true, y_predict, average_method='arithmetic'))
    
    # Y_sklearn = TruncatedSVD(n_components=2).fit_transform(tfidf_matrix.todense())
    
    # plt.scatter(Y_sklearn[:, 0], Y_sklearn[:, 1], alpha=0.5)
    # plt.show()

    # unique_labels = set(y_predict)
